...
点击其中一个会自动结束另一方的轮次，同一时刻只有一条链路在跑。你说的每句话念完后，气泡上会出现「重新朗读」，对方没听清时可以重放。
...
对话的两个方向共用同一支麦克风，而 Gummy 的翻译目标语言**在建立连接时就固定了**，一条连接只能是「外语→中文」或「中文→外语」其中之一，无法中途切换。

所以不存在两条链路同时开着的选项。这个限制反而规避了三个真实问题：

1. **交叉污染**。若两条链路同时开着，你说的中文会被「外语→中文」那条也听进去，按外语强行拟合成乱码。
2. **成本翻倍**。Gummy 的识别与翻译分别计费，开两条连接就是四份费用。
3. **回声循环**。扬声器播放译文时，麦克风会把这段声音重新收进来。代码里通过播放期间静音上行音频来切断（`ListenSession.mute()`），这一点有专门的测试覆盖 —— 它在真机上不容易稳定复现，但一旦失效，对方会听到系统把自己刚说的话又翻译一遍。
...
这是回声。正常情况下播放译文时麦克风上行会被自动掐断，如果仍然出现，可能是外放音量过大或麦克风灵敏度过高，建议改用耳机。
